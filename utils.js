...
export const vip={
  is:()=>!!(ls.get("ibg.vip")||ls.get("ibg.sub")==="active"),
  markLifetime:()=>ls.set("ibg.vip",true),
  markSub:()=>ls.set("ibg.sub","active")
};
...
export const unlock={
  set:()=>new Set(ls.get("ibg.unlocked",[])),
  has:(id)=>new Set(ls.get("ibg.unlocked",[])).has(id)||vip.is(),
  add:(id)=>{ const s=new Set(ls.get("ibg.unlocked",[])); s.add(id); ls.set("ibg.unlocked",[...s]); }
};
...
if (typeof window!=="undefined"){
  window.Utils = { $, $$, money, ls, vip, todaySeed, seedToday, seededShuffle, shuffleSeeded, pickN, uidFromPath, unlock, hasUnlock, addUnlock, getUnlocks };
}
