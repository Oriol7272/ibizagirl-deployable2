/* Usa EXOCLICK_ZONES (DISPLAY). Nunca STICKY aquí. */
(function(){
  function mount(){
    ...
    BG_ADS.ensureProvider(function(ok){
      ...
      function go(zone, host, where){
        BG_ADS.mountExo(zone, host);
        console.log("BG_ADS: EXO/AP mounted (display) ->", zone, "on", where);
  ...
  BG_ADS.waitForEnv(["EXOCLICK_ZONES"], function(ok){
