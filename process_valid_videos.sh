...
echo "Processing remaining valid videos (excluding system files)..."

current_count=$(ls processed_videos/ | wc -l)
echo "Currently processed: $current_count videos"

count=0
find extracted_arxiu/ -name "*.mp4" -not -path "*__MACOSX*" -not -name "._*" | while read video_path; do
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "✓ Processed $new_total/70: $filename"
    else
...
echo "Processing complete! Total videos: $final_count/70"