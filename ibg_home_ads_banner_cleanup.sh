...
echo "[IBG] Cleanup: hero sin overlay + ads laterales/inline con tamaño y placeholder + deploy"

# 1) CSS — quitamos capa oscura, ajustamos ads y placeholders
cat > css/ibg-home.css <<'CSS'
/* ---------- HERO (sin overlay, solo fondo rotando detrás del texto) ---------- */
...
/* ---------- Carrusel (grande, autoplay ya en JS) ---------- */
...
/* Que no salgan “caracteres raros” si un script escribe texto */
.side-ad, .ad-inline{font-size:0;line-height:0;color:transparent}

/* Placeholder discreto cuando no hay creatividades */
.ad-placeholder{width:100%;height:100%;display:flex;align-items:center;justify-content:center;
  border:1px dashed rgba(255,255,255,.18); border-radius:10px; font-size:14px; line-height:1; color:rgba(255,255,255,.6)}
...
# 2) HOME — quitamos <div class="hero-overlay"> del HTML
...
# 3) Ad-loader — inyecta en left/right (160x600) e inline (728x90) y pone placeholder si no carga nada
...
git add -A
git commit -m "HOME: remove overlay; stronger ads injection (sizes + placeholders); keep centered layout"
