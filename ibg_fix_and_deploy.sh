...
echo "== 2) Hotfix CSS (artefacto header + oculta SOLO huecos vacíos) =="
node - <<'NODE'
const fs=require('fs');
const f='index.html';
...
const css = `
  <style id="ibg-cleanup-css">
    ...
    /* mostrar SOLO contenedores buenos */
    ...
    /* ocultar placeholders y filas de huecos */
    .ad-placeholder, .ad-slot-empty, .ad-empty, .dummy-ad, .ghost-ad,
    .ad-bottom-row, .bottom-row-ads, .ads-row {
...
git add -A
git commit -m "fix(home): env ads, rutas /js|/css, shims anti-404, hotfix css, noop initAds" || echo "ℹ️ Nada que commitear."
