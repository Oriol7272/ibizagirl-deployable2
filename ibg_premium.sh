...
function pickRandom(arr,n){ var pool=arr.slice(); for(var i=pool.length-1;i>0;i--){ var j=Math.floor(Math.random()*(i+1)); var tmp=pool[i]; pool[i]=pool[j]; pool[j]=tmp; }
...
function isSub(){ return isSubscribed() || localStorage.getItem('ibg_subscribed')==='1'; }
...
export async function initPremium(){
  ...
  app.innerHTML =
    ...
    '<div id="pp-modal"><div class="box"><div id="paypal-container"></div></div></div>';
  ...
  function renderCredits(){
    ...
    el.textContent = sub ? 'Suscripción activa: acceso a todo el contenido.' :
      (c>0 ? ('Créditos disponibles: '+c+' (1 crédito = 1 imagen)') : 'Sin créditos. Puedes comprar una imagen (0,10 €) o un paquete de 10 (0,80 €).');
  }
  renderCredits();
  ...
  function buyOne(url, card){
    // si hay sub o crédito, no abrir PayPal
    if (isSub()) {
      unlock(url, card);
      return;
    }
  ...
  grid.addEventListener('click', (e)=>{
    const btn = e.target.closest('.buy-one');
    if(!btn) return;
  ...
  document.getElementById('sub-month').addEventListener('click', ()=>{
    const plan = (window.IBG||{}).PAYPAL_PLAN_MONTHLY_1499;
    if(!plan || !window.IBGPay) return;
    window.IBGPay.subscribe(plan, ()=>{
      markSubscribed();
      // desbloquear todo lo visible
      document.querySelectorAll('.p-card.locked').forEach(el=>el.classList.remove('locked'));
      renderCredits();
    });
  });
  document.getElementById('sub-year').addEventListener('click', ()=>{
    const plan = (window.IBG||{}).PAYPAL_PLAN_ANNUAL_4999;
    if(!plan || !window.IBGPay) return;
    window.IBGPay.subscribe(plan, ()=>{
      markSubscribed();
      document.querySelectorAll('.p-card.locked').forEach(el=>el.classList.remove('locked'));
