(function(){
  function getVars(){
    return {
      client:  (window.PAYPAL_CLIENT_ID||""),
  ...
  function loadPayPal(cb){
    var v=getVars();
    if(!v.client){ console.error("PAYPAL_CLIENT_ID vacío"); alert("Falta configurar PayPal (PAYPAL_CLIENT_ID)."); return; }
    ...
    s.onload=function(){cb()}; document.head.appendChild(s);
  }
  ...
  function mountSubs(){
    var v=getVars();
    [['btn-monthly',v.monthly],['btn-yearly',v.yearly],['btn-lifetime',v.life]].forEach(function(pair){
      var id=pair[0], plan=pair[1], el=document.getElementById(id); if(!el) return;
      ...
      loadPayPal(function(){
        window.paypal.Buttons({
          ...
          onApprove:function(d,a){ alert("Suscripción activada"); }
        }).render('#'+id);
      });
    });
  }
