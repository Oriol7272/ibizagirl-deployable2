(function(){
  ...
  function startRotation(list){
    ...
    if(!list || !list.length){ bannerImg.style.backgroundImage='url("decorative-images/paradise-beach.png")'; bannerImg.style.opacity=.28; return; }
    var i=0;
    function rot(){ bannerImg.style.opacity=0; var src=list[i%list.length];
      setTimeout(function(){ bannerImg.style.backgroundImage='url("'+src+'")'; bannerImg.style.opacity=.28; },240); i++; }
    rot(); setInterval(rot,4000);
  }
  document.addEventListener('DOMContentLoaded',function(){
    fetch('decorative-images/manifest.json').then(function(r){return r.ok?r.json():null})
    .then(function(j){var arr=j&&Array.isArray(j.images)?j.images:[]; var list=arr.map(n=>'decorative-images/'+n);
    ...
    var grid=$('#grid40'); if(grid){g40.forEach(function(src){var d=document.createElement('div'); d.className='thumb'; var img=new Image(); img.src=src; d.appendChild(img); grid.appendChild(d);});}
  });
})();
