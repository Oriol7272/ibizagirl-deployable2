(function(){
  ...
  function pickObjSrc(o){
    ...
    for (var i=0;i<keys.length;i++){
      var v=o[keys[i]]; if (typeof v==='string' && isImgStr(v)) return v;
    } return null;
  }
  function autodetectPublicDeep(){
    ...
    function scoreArray(arr){
      ...
      var ratio=(hit+objHit)/L; if (ratio<0.5) return -1;
    ...
    function walk(o, path, d){
      ...
      if (Array.isArray(o)){ var sc=scoreArray(o); if (sc>(best?best.sc:-1)) best={sc:sc,arr:o,path:path}; return; }
      ...
      for (var i=0;i<ks.length;i++){
        ...
        if (Array.isArray(v)){ var sc=scoreArray(v); if (sc>(best?best.sc:-1)) best={sc:sc,arr:v,path:path+"."+k}; }
        else if (v && typeof v==='object'){ walk(v, path+"."+k, d+1); }
      }
    }
    try{ walk(W,"window",0);}catch(e){}
    if (best && best.arr){
      var out=[]; for (var i=0;i<best.arr.length;i++){
        var it=best.arr[i]; if (isImgStr(it)) out.push(it); else { var s=pickObjSrc(it); if (isImgStr(s)) out.push(s); }
      }
      if (out.length){ W.CONTENT_PUBLIC=out; console.log("🔎 CONTENT_PUBLIC autodetectado en",best.path,"("+out.length+" items)"); return true; }
    }
    console.warn("⚠️ No se pudo autodetectar CONTENT_PUBLIC");
    W.CONTENT_PUBLIC=W.CONTENT_PUBLIC||[]; return false;
  }
