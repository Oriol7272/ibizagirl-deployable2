(function(){
  ...
  try{
    ...
    if (!document.getElementById('sub-root')){
      ...
      sec.innerHTML=`
        <h2 style="font:700 26px system-ui;margin:0 0 10px;">${PP_ICON}Suscripciones</h2>
        <div style="display:grid;gap:14px;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));">
          <div style="border:1px solid #ddd;border-radius:12px;padding:16px;"><div style="font-weight:700;margin-bottom:6px;">${PP_ICON}Mensual · 14,99€</div><div id="sub-monthly"></div></div>
          <div style="border:1px solid #ddd;border-radius:12px;padding:16px;"><div style="font-weight:700;margin-bottom:6px;">${PP_ICON}Anual · 49,99€</div><div id="sub-yearly"></div></div>
          <div style="border:1px solid #ddd;border-radius:12px;padding:16px;"><div style="font-weight:700;margin-bottom:6px;">${PP_ICON}Lifetime · 100€ (pago único)</div><div id="sub-lifetime"></div></div>
        </div>
        <div style="margin-top:8px;font:12px system-ui;opacity:.8;">Acceso ilimitado mientras dure la suscripción. Lifetime = acceso vitalicio.</div>
        <div id="sub-status" style="margin-top:6px;font:12px system-ui;opacity:.8;"></div>
      `;
    ...
    loadSDK({ 'client-id':CID, currency:CUR, intent:'subscription', vault:'true', components:'buttons' })
      .then(()=>{
        ...
        if (PLAN_L){ sub(document.getElementById('sub-lifetime'), PLAN_L); }
        else {
