...
export function renderCarousel(el, items){
  ...
  items.forEach(it=>{ const d=document.createElement('div'); d.className='carousel-item card'; const img=document.createElement('img'); img.loading='lazy'; img.src=it.thumb||it.src; img.alt=''; d.appendChild(img); track.appendChild(d); });
...
export function renderGrid(el, items, {withPrice=false, lock=true, kind='photo'}={}){
  ...
  items.forEach(it=>{
    ...
    if(!unlocked){
      const buy=document.createElement('button'); buy.className='buy-btn'; buy.dataset.id=it.id; buy.dataset.kind=(it.type==='video'?'video':'photo'); buy.innerHTML='<span class="pp-icon"></span><span>PayPal</span>'; card.appendChild(buy);
    }
...
export function setCounter(sel, total, news){
  const c=qs(sel); if(!c) return; c.textContent = (typeof news==='number') ? ('Novedades de hoy: '+news+' · Total: '+total) : ('Total: '+total);
}
